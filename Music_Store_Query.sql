...
SELECT c.customer_id, concat(c.first_name," ", c.last_name) as Name, SUM(i.total) AS total_spending
FROM customer c
...
SELECT DISTINCT c.email AS Email,c.first_name AS FirstName, c.last_name AS LastName, g.name AS Genre_Name
FROM customer c
...
/*Method 1 */
select a.artist_id, at.name as Artist_Name,count(a.album_id) as No_of_Albums,g.name as Genre_Name from album2 a 
...
select track_id, name, milliseconds from track t 
